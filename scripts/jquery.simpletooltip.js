...
;(function($){
	...
	$.fn.simpletooltip = function(settings) {		
		...
		this.each(function () {		
			...
			if (options.click) {
				...
				.bind("mouseleave", function() {
					if ( options.centerTooltip == false ){
						$this.data('$tooltip').trigger('hide');
					}
		...
		function initTooltip($tooltip) {		
			...
			// Set CSS
			.css({"position": "absolute", "zIndex": "998" , "display": "none"})			
			// Close tooltip btn
			...
			// Init ok.
			.data("init", true);			
			$(document).keypress(function(e){ $tooltip.trigger("hide"); });
		};		
		function openTooltip(e){			
			...
			if (opts.floatingTooltip == true){
				var tooltipPosX = $target.offset().left + $target.width();
				var tooltipPosY = ($target.offset().top + ($target.outerHeight()/2)) - ($tooltip.data("height")/2);
				if (e.pageX == 0) {
					tooltipPosX = $target.offset().left + $target.width();
				...
				if (opts.centerTooltip == true) {
					...
					var tooltipPosX = $target.offset().left + ($target.outerWidth()/2) - ($tooltip.data("width")/2);
			...
			if (opts.floatingTooltip == false){
				...
				if (tooltipPosX + $tooltip.data("width") > (winOffsetX + winWidth - opts.margin)) { // Right
					tooltipPosX = $target.offset().left - $tooltip.data("width") - opts.margin;
					$tooltip.addClass('floating-tooltip-right');
				} else { // left
					tooltipPosX = $target.offset().left + $target.outerWidth() + opts.margin;
					$tooltip.removeClass('floating-tooltip-right');
				}
			}						
			if (opts.centerTooltip == false){
				$tooltip.removeClass('tooltip-bottom');
				if (tooltipPosY < winOffsetY + opts.margin) { // Bottom	
					tooltipPosY = $target.offset().top + $target.outerHeight();
					$tooltip.addClass('tooltip-bottom');
				} else if (tooltipPosY + $tooltip.data("height") > (winOffsetY + winHeight - opts.margin)) { // top
